...
const EC_LEVEL_TO_NUM: Record<EcLevel, number> = { L: 0, M: 1, Q: 2, H: 3 };
...
/**
 * テキストを QR 行列にエンコードする。
 * 容量超過・空文字・不正オプション（version が 1..40 の範囲外）は null を返す。
 */
export function encode(text: string, options?: EncodeOptions): QrMatrix | null {
  const ecLevel = options?.ecLevel ?? "M";
  const version = options?.version ?? 0;
  const ecNum = EC_LEVEL_TO_NUM[ecLevel];
  // TS の型では EcLevel は "L"|"M"|"Q"|"H" に閉じているが、非TS呼び出し元が
  // 型を無視して不正な値を渡すケースに備え、total function として扱う（範囲外は null）。
  if (ecNum === undefined) {
    return null;
  }
