...
4. 成功した総縮小率（事前半減込み）を結果表示に反映する（例:「読取成功（1/8スケール）」）。
...
- `node --test packages/moonqr/test/*.test.mjs`: **272/272 PASS**（既存268 + 新規
  `monitor-lattice.test.mjs` 4件。4件目は事前半減のスケール計上回帰テスト）
