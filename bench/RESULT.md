...
ラン区間の文字数・UTF-8バイト数・必要モードをprefix集計し、DPの各遷移をO(1)にした。
また、文字数指示子が変わる3バンド（1-9 / 10-26 / 27-40）ごとに分割結果を1回だけ求めて
version試行間で再利用する。`"a1"` を500回繰り返した1,000文字の交互ラン入力は、修正前
21,612msから修正後50msへ短縮した（同一環境、Node v24.18.0）。回帰テストの上限は、
環境差の余裕を含めて5,000msとしている。

Model 2の絶対最大容量（version 40-L Numericの7,089文字）を超える入力はDP前に棄却する。
`"a1"` を10,000回繰り返した20,000文字について、auto/明示version 40の両方を合わせた
棄却時間は修正前13,223msから修正後0.1msへ短縮した。
