...
> **このファイルのポリシー**: 本ファイルは各フェーズの実測結果の記録である。再計測で数値が
> 変わった場合は、該当節を**置換**する（旧数値の下に新数値を追記しない — 差分は git history
> で追える）。パリティ/性能ゲートの合否判定そのものはテスト内アサート（`moon test` /
> `node --test` 等）が正本であり、本ファイルはその根拠（環境・方法・実測値）の保存先である。
...
## 混在モードのセグメント最適化（2026-07-29）

### 方法

- 比較対象: `qrcode` npm 1.5.4。
- 入力: `packages/moonqr/test/segment-optimization.test.mjs` の11入力。小文字を含むURLと
  5/10/30/100桁の数字列、日本語と30桁の数字列、英数字のみ、数字のみ、短い混在、
  複数の数字列を含むSKUを含めた。
- ECレベル: L / M / Q / H の4種。合計 **11入力 × 4 EC = 44ケース**。
- moonqr は version 1〜40を線形試行して最初に収まるversionを求め、`qrcode` の自動選択
  versionと比較した。1件でも moonqr のversionが大きければ不合格とした。
- 正しさは全11入力をEC Mで生成し、自前デコーダと独立実装のjsQRの両方で元テキストへ
  復元できることを確認した。

### 結果

- qrcode npmより大きいversionになったケース: **0 / 44**（PASS）。
- 自前デコーダ往復: **11 / 11**（PASS）。
- jsQR往復: **11 / 11**（PASS）。
- 代表ケース（EC M）: `https://ex.com/id/` + 100桁の数字は、分岐元の単一Byteモード
  **v7（45×45）** から混在モード **v4（33×33）** へ縮小した。

### encoderバンドルサイズ

分岐元 `87ae3d9` と同じtoolchain・依存で `./encode` CJS出力を比較した。判定基準は実配信
コストであるgzipの増加が1KB未満であること。

| | raw | gzip |
|---|---:|---:|
| 分岐元 | 21,763 B | 6,539 B |
| 最適化後 | 25,001 B | 7,492 B |
| 差分 | +3,238 B | **+953 B** |

gzip増加は **953 B < 1KB** でPASS。raw増加はアルゴリズムのコードであり、漢字変換表の
ような大きなデータはencoderへ持ち込んでいない。

### 計算量回帰（2026-08-28: O(n) DP）

隣接 run のモードが必ず異なる構造を利用し、全開始点を走査する O(n²) DP を O(n) へ
置き換えた。Byte は全開始点の変換済み最小値、Alphanumeric は直近の Byte run 以降に
ある開始文字位置の偶奇別最小値、Numeric は直前の Numeric run 固定として各終点を O(1)
で処理する。コストは Numeric の3文字周期と Alphanumeric の2文字周期を正確に扱うため、
6倍スケールの整数で比較している。

#### 環境と方法

- 環境: macOS 26.3.1 / arm64、Node v24.20.0、MoonBit
  `0.1.20260713 (75c7e1f 2026-07-13)`。
- 比較対象: before は分岐元 `1cb32e9`、after は O(n) DP 実装後。同じマシン上でそれぞれ
  `moon build --target js --release` した生成物を直接 `require` した。
- 各入力でウォームアップ1回後、`encode_js(text, 0, 0)` を7回計測し、中央値を採用した。
  戻り値も確認し、収まる入力は選択 version、容量超過は `[]` であることを記録した。

| 入力 | 文字数 / run数 | before median | after median | 改善 | 結果 |
|---|---:|---:|---:|---:|---|
| `"a1".repeat(250)` | 500 / 500 | 12.3ms | 6.6ms | 1.9倍 | v15 |
| `"a1".repeat(1000)` | 2,000 / 2,000 | 100.1ms | 27.1ms | 3.7倍 | v33 |
| `"a1".repeat(2000)` | 4,000 / 4,000 | 287.5ms | 10.6ms | 27.1倍 | `[]` |
| `"a1".repeat(3544)` | 7,088 / 7,088 | 889.4ms | **22.1ms** | **40.2倍** | `[]` |
| `"1A".repeat(2678)` | 5,356 / 5,356 | 799.5ms | **11.6ms** | **68.9倍** | `[]` |
| `"1".repeat(7089)` | 7,089 / 1 | 50.9ms | 48.4ms | 1.1倍 | v40 |

性能回帰テストは、最悪 run 入力2件をJITウォームアップ後に個別計測し、どちらも100ms未満を
要求する。単一 run の7,089文字がほぼ不変である対照から、文字数ではなく run 数に依存して
いた二乗コストを除去した効果だと確認できる。既存の1,000文字 / 5,000ms上限と auto / forced
の選択 version 一致検査も維持している。
